...
| **TEND** | `seed-tend.yml` — the board lane: survey open PRs/issues, escalate what has burned its repair budget, repair what is red or conflicted, guard what that repair pushed, merge what is provably green, dispatch approved issues, close resolved CI-failure issues. `seed-steward.yml` — the `@claude` mention handler. `seed-evolve.yml` — the issue lane (`seed:request` intake → a human applies `seed:approved` → implementation draft PR; `seed:hold` is the brake) | Cron + called by every grow tick; mentions; labels | `SEED_TEND_ENABLED` for the board lane, `SEED_EVOLVE_ENABLED` for the issue lane; a mention is its own consent |
...
**The repair budget and its terminal state.** `policy.board.max_repairs_per_run` bounds one tick's spend; `policy.board.max_repair_attempts` bounds a single PR's lifetime. Each repair pass leaves a marker comment on the PR it touched, so the attempt count lives in the PR thread — auditable by a human reading it, and needing no state outside GitHub. A PR that exhausts its budget is escalated: the lane applies `guardrails.merge_blocked_by_label`, comments why, and never repairs it again. That one label is terminal in three directions at once — the survey skips blocked PRs, the merge step treats it as a hard stop, and (because it is also an `ignore_labels` entry) the board stops counting it, so an unfixable PR cannot freeze growth. An attempt is recorded only when the repair pass ran to completion: the step is `continue-on-error`, and charging a PR for an auth outage would escalate work that was never broken.

**The diff guard.** The repair prompt forbids touching `.github/workflows/`, `.github/actions/`, `.seed/pause.yml`, and `.seed/seed.yml` — but a prompt is an instruction and only the pushed diff is evidence. The survey records each repairable PR's head SHA before the pass; afterwards a deterministic step compares it against the live head, reads the changed files through the compare API, and escalates any PR whose repair reached a protected path. The merge step's own hard stop already refuses workflow-touching PRs; the guard is what makes the breach visible and terminal rather than silently green.
...
3. **PR-only, and merging is confined to one lane.** Autonomous writes land on `seed/*` branches as draft PRs; review approvals and pushes to the default branch are forbidden to every lane, always. No *model pass* may ever merge. Merging happens only in `seed-tend.yml`'s deterministic step, and only when every hard stop in `policy.merge` clears: all checks green, no conflicts, a seed- or bot-authored branch, and no block label (`human-review`, `seed:hold`, `do-not-merge`). A PR failing any one of those is left open for a human — the designed outcome, not a failure. `seed.py check` errors if any other `seed-*` workflow contains `gh pr merge`, and errors if a repo that permits merging drops the `merge_requires_green` / `merge_human_authored` hard stops, and errors if the installed `seed-tend.yml` loses one of its guardrail markers — the consent gate, the kill switch, the never-auto-merge stop for CI-touching PRs, the live-state re-verification, the repair budget, the attempt ledger, or the diff guard. Set `guardrails.never_merge: true` to forbid merging outright.
...
- **Per-run triage**: `triage-on-failure.yml` watches the named workflows via `workflow_run`, downloads failed-run logs, and files a triage issue. The issue **title is the dedupe key** — `[CI Failure] <workflow> on <branch>`, deliberately free of the commit SHA and with the ref normalized (`refs/heads/main` → `main`), so a recurring failure updates one issue instead of minting a new pair every push. The tend lane closes it once the signal it named has cleared — checked against **the branch in the title**, not the default branch (asking the default branch about a feature-branch failure answers a different question and returns `unknown` forever, which is how the first version left a permanent floor of unclearable issues). Four ways the signal clears: the workflow no longer exists, the branch no longer exists, its latest run on that branch is green, or the issue carries the retired per-commit title and has been superseded. Filer and closer together are what let the board converge; a filer alone only ratchets. (Fleet caveat honored: `workflow_run` chains are fragile for *sequencing* — prefer `needs:` inside a pipeline; `workflow_run` here is pure observation, the one job it is fit for.)
