...
| Clear the board before starting new work (tend → survey → grow) | [bamr87/bamr87](https://github.com/bamr87/bamr87) `fleet-pulse.yml` (the daily loop triages the failing + expensive queue before anything else) and lifehacker.dev's fix-before-publish ordering | Half-finished work is not a base to build on; a queue that only grows stops being a queue |
| Automated CI triage and repair of the loop's own PRs | bamr87 hub `fleet-pulse.yml` `doctor` (reads each candidate's logs and opens the fix) | An autonomous producer that cannot fix its own red PRs just moves work onto the human |
| Policy-gated auto-merge with per-condition hard stops | [bamr87/gitorio](https://github.com/bamr87/gitorio) (Dependency Warden auto-merged vetted Dependabot PRs; merge policy hardened in ADR-015 after an incident) | Every condition is a hard stop, not a heuristic — and the failure mode is "left open for a human", never "merged anyway" |
| Stable dedupe key so a recurring failure is ONE issue, not a stream | [bamr87/wtd](https://github.com/bamr87/wtd) (stable `<!-- wtd-fleet:<key> -->` markers — "rescans converge, never duplicate") and lifehacker.dev's fingerprinted findings | A key containing the commit SHA is not a key; it mints a new issue per push until the board is unusable |
| Machine-filed tickets are closed by the machine when the signal clears | bamr87 hub `remediation.py` (dedupes against open issues *and* PRs, so a resolved item stops re-filing) | An autonomous filer without an autonomous closer only ratchets; the queue must be able to go down |
| A PR that edits CI never auto-merges | [bamr87/wtd](https://github.com/bamr87/wtd) (`.github/workflows/` unwritable by agents) + gitorio's least-privilege compilation | Workflows run with the repo's secrets, so merging one is a privilege change — the one hard stop no policy may opt out of |
| Merging confined to ONE auditable lane | gitorio ADR-014/016 (`sink.merge_pr` is a distinct machine, never folded into an agent) | The lane that merges is deterministic and greppable; no model pass carries the capability |
