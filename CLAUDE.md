...
- **The repair loop has a terminal state** (kernel v0.5.0): `policy.board.max_repair_attempts` bounds how many times the tend lane will try to fix one PR (the count lives in the PR thread as marker comments). Out of budget → the block label, which stops the retries, stops auto-merge, and parks it. A deterministic guard then re-reads what each repair actually pushed and escalates anything that reached `.github/workflows/`, `.github/actions/`, `.seed/pause.yml`, or `.seed/seed.yml` — the prompt is the instruction, the diff is the evidence.
